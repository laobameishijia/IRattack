...
bool FlatPlus::doFlat(Function &F) {
    ...
    LoadInst *label = dispatcherBuilder.CreateLoad(labelPtr->getAllocatedType(), labelPtr);
    ...
    for (BasicBlock *bb : useful) {
        ...
        if (isa<InvokeInst>(bb->getTerminator())) {
            ...
            auto v1 = trampolineBuilder.CreateLoad(yPtr->getAllocatedType(), yPtr);
            auto v2 = trampolineBuilder.CreateXor(v1, xTarget ^ yNow);
            trampolineBuilder.CreateStore(v2, xPtr);

            auto v3 = trampolineBuilder.CreateLoad(xPtr->getAllocatedType(), xPtr);
        ...
        case 1: {
            ...
            auto v1 = caseBuilder.CreateLoad(yPtr->getAllocatedType(), yPtr);
            auto v2 = caseBuilder.CreateXor(v1, xTarget ^ yNow);
            caseBuilder.CreateStore(v2, xPtr);

            auto v3 = caseBuilder.CreateLoad(xPtr->getAllocatedType(), xPtr);
        ...
        case 2: {
            ...
            if (BranchInst *endBr = dyn_cast<BranchInst>(bb->getTerminator())) {
                ...
                auto v5 = caseBuilder.CreateLoad(xPtr->getAllocatedType(), xPtr);
...
BasicBlock **FlatPlus::genTransBlocks(Function &F, Value *xPtr,
    ...
    for (size_t i = 0; i < subTransCnt; i++) {
        ...
        if (i == 0) {
            // first trans block
            builder.CreateStore(builder.CreateLoad(xPtr->getType()->getPointerElementType(),xPtr), bakPtr[0]);
            builder.CreateStore(builder.CreateLoad(yPtr->getType()->getPointerElementType(),yPtr), bakPtr[1]);
            builder.CreateStore(builder.CreateLoad(xPtr->getType()->getPointerElementType(),xPtr), yPtr);
            builder.CreateStore(builder.getInt32(imm32), xPtr);
        }
        auto v0 = builder.CreateLoad(xPtr->getType()->getPointerElementType(),xPtr);
        auto v1 = builder.CreateLoad(yPtr->getType()->getPointerElementType(),yPtr);
        ...
        if (i == subTransCnt - 1) {
            // last trans block
            builder.CreateStore(builder.CreateLoad(bakPtr[0]->getAllocatedType(), bakPtr[0]), xPtr);
            builder.CreateStore(builder.CreateLoad(bakPtr[1]->getAllocatedType(), bakPtr[1]), yPtr);
