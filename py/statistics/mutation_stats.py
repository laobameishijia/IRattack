...
def get_model_names(folder_path, model):
    ...
    for filename in os.listdir(folder_path):
        ...
        if filename.startswith('success'):
            ...
            if match:
                ...
                itertion = int(match.group(2))
                if itertion == 0: itertion += 1
                if model_name == model:
                    full_path = os.path.join(folder_path, filename)
                    return (full_path, model_name, itertion)
...
if __name__ == '__main__':
    ...
    for malware_dir in malware_full_paths:
        res = get_model_names(f"{malware_dir}/out", model)
        if res != None:
    ...
    avg_itertion = 0
    for i in mutation['itertion']:
        avg_itertion += i / len(mutation['itertion'])
    
    # 对每个子字典进行排序
    mutation = {k: sort_dict_by_value(v) for k, v in mutation.items() if k != "itertion"}    
    
    # 保留两位小数
    avg_itertion = round(avg_itertion, 2)
    mutation['avg_itertion'] = avg_itertion
